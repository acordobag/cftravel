...
import { PriceObject } from '../entities/entities';

export interface TransferPriceBreakdown {
  routeDistance: number;
  routeRate: PriceObject;
  routeSubtotal: number;
  routeDiscountAmount: number;
  repositionDistance: number;
  repositionRate: PriceObject;
  repositionSubtotal: number;
  total: number;
}
...
export class PricingService {
  ...
  getKilometerRate(distance: number): PriceObject {
    for (let i = 0; i < this.rateRules.length; i++) {
      const rule = this.rateRules[i];
      if (distance > rule.min && distance <= rule.max || distance === 0 && rule.min === 0) {
        return { price: rule.price, discount: rule.discount };
      }
    }

    return { price: 0.9, discount: 0 };
  }

  calculatePrivateTransferRate(routeDistance: number, repositionDistance: number): TransferPriceBreakdown {
    const routeRate = this.getKilometerRate(routeDistance);
    const repositionRate = this.getKilometerRate(repositionDistance);
    const routeSubtotal = this.roundMoney(routeDistance * routeRate.price);
    const routeDiscountAmount = this.roundMoney(routeSubtotal * routeRate.discount);
    const repositionSubtotal = this.roundMoney(repositionDistance * repositionRate.price);
    const total = this.roundMoney(Math.max(routeSubtotal - routeDiscountAmount, 0) + repositionSubtotal);

    return {
      routeDistance: this.roundDistance(routeDistance),
      routeRate: routeRate,
      routeSubtotal: routeSubtotal,
      routeDiscountAmount: routeDiscountAmount,
      repositionDistance: this.roundDistance(repositionDistance),
      repositionRate: repositionRate,
      repositionSubtotal: repositionSubtotal,
      total: total
    };
  }
